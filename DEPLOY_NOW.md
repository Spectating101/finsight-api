...
4. **Create endpoint:** `https://finsight-api-prod.herokuapp.com/api/v1/webhooks/stripe`
...
2. **Create New Project**
   - Platform: Python
   - Project name: `finsight-api-prod`
   - Team: Default
...
```bash
heroku config --app finsight-api-prod
```
...
```bash
curl https://finsight-api-prod.herokuapp.com/health
```
...
Visit: `https://finsight-api-prod.herokuapp.com/docs`
...
curl -X POST https://finsight-api-prod.herokuapp.com/api/v1/auth/register \
...
curl -H "X-API-Key: YOUR_KEY_HERE" \
  "https://finsight-api-prod.herokuapp.com/api/v1/answers?ticker=AAPL&metric=revenue_ttm&format=json"
```
...
```bash
# Check Redis URL
heroku config:get REDIS_URL --app finsight-api-prod

# Should start with rediss:// (TLS)
# If not, Redis addon might not be configured
heroku addons:info heroku-redis --app finsight-api-prod
```
